...
Basic design: takes parameters (volumes, number of tubes/dilutions) in the form of list variables;
lists are then parsed accordingly
(example: a list of tubes [7, 1, 6] means 7 tubes of one dilution factor, one tube of a second dilution factor,
and six tubes of a third dilution factor)
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    # Set up parameters:
    tube_counter = 1 #keep track of total number of tubes
    index = 0 #keep track of index within "diluent_vol" to access
    num_tubes[0] = num_tubes[0] - 1 #get rid of first tube in list - this first tube is the stock tube

    p300.pick_up_tip()
    
    for j in num_tubes: #iterate through list of different tube numbers

        for i in range(tube_counter, j + tube_counter): #iterate from "counter" - the last filled tube - to end
            if tube_counter > sum(num_tubes): #break loop if we've iterated past number of available tubes to fill
                break
            p300.transfer(
                diluent_vol[index], #use appropriate item within list - match to "j" in num_tubes
